...
def validate_position(st, direction, position,last):
    if direction == -1 and 0 <= position[0, 1] - 1 < 3 and last != st[position[0, 0], position[0, 1] - 1]:
        st[position[0, 0], position[0, 1]], st[position[0, 0], position[0, 1] - 1] = st[position[0, 0], position[0, 1] - 1], st[position[0, 0], position[0, 1]]
        position[0, 1] -= 1
        ...
        st[position[0, 0], position[0, 1]], st[position[0, 0] + 1, position[0, 1]] = st[position[0, 0] + 1, position[0, 1]], st[position[0, 0], position[0, 1]]
        position[0, 0] += 1
...
start = time.time()
solution = iddfs(st, initial_position, max_depth=4)
end = time.time()
